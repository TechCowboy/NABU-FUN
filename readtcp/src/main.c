...
void main() 
{
  ...
  uint8_t chars_read, chars_written;
  int     i;
  ...
  if (ImHosting)
  {
    while (true)
    {
      ...
      chars_read = rn_TCPServerRead(_inBuf, 0, IN_BUF_LEN);
      // it appears that 255 will be returned if the
      // connection is lost
      if (chars_read == 255)
        break;
  ...
  {
    char default_url[] = {"192.168.2.100"};
    char url[32] = { "" };
    uint16_t port = 6502;
    ...
    printf("IP address of echo server:\n");
    //gets(url);
    url[strlen(url)-1] = '\0';
    if (strcmp(url,"") == 0)
    ...
    while (true)
    {
      ...
      // This is just to slow down the number
      // of packets sent.
      ...
      chars_written = rn_TCPHandleWrite(tcpHandle, 0, strlen(temp), temp);
